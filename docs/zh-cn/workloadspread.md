...
Subset对应一个`maxReplicas`数量。WorkloadSpread利用Webhook注入Subset定义的域信息，且控制Pod的扩缩容顺序。

目前支持的Workload类型：`CloneSet`、`Deployment`、`ReplicaSet`、`Job`。
...
```
**注意**：`TargetReference`不可以变更，且单个WorkloadSpread只能管理一个Workload。
...
`MaxReplicas`：当前版本暂不支持百分比；可以设置为空，代表不限制subset的副本数。

`Patch`: 自定义Subset的Pod配置，可以是Annotations、Labels、Env等。
...
**模拟调度**： Kruise会对Subset的Node资源做简单过滤，若资源不足会调度到下一个Subset。
  
**Reschedule**：Kruise检查Subset调度失败的Pod，若超过用户定义的时间就调度到其他Subset。

## 环境要求

### Pod Webhook
WorkloadSpread 需要Pod webhook来注入规则并且关心Pod的更新和删除。
...
## 扩缩容顺序：
### 扩容
- 按照Subset定义的顺序调度Pod，当前Subset的Pod数量达到maxReplicas时再调度到下一个Subset。
  
### 缩容
- 当Subset的副本数大于定义的maxReplicas时，优先缩容多余的Pod。
- 依赖于Subset中声明的顺序，后面的Subset先于前面的被删除。
...
3. 如zone副本分布需要变动，先调整对应Subset的`maxReplicas`，再调整workload副本数。